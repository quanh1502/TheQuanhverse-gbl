...
import { 
  ...
  Link as LinkIcon, Play, Calendar, AlignLeft, Wand2, Loader2,
...
// --- COMPONENTS (JewelCase3D, AddNewAlbum...) ---
...
const AudioRoom: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, "audio-shelves"), 
      (snapshot) => {
        ...
        // Sắp xếp theo ID để tránh bị nhảy vị trí
  ...
  // --- 2. CÁC HÀM GHI DỮ LIỆU (Có xử lý lỗi) ---
  ...
  const handleAddNewItem = async (shelfId: number) => {
     try {
       ...
       if (shelf) {
         ...
         // Tự động mở rộng kệ khi thêm mới để user nhìn thấy item
            ...
            {isLoading ? (
               ...
               <>
                 ...
                 {shelves.map((shelf) => {
                    ...
                    const visibleItems = isExpanded ? shelf.items : shelf.items.slice(0, PREVIEW_LIMIT);
                    ...
                    <div key={shelf.id} className="relative group/shelf" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, shelf.id)}>
                       ...
                       <div className="flex items-center gap-4 mb-6 border-b border-cyan-900/50 pb-2 w-full max-w-md">
                          ...
                          {editingShelfId === shelf.id ? (
                             ...
                             <div className="flex items-center gap-4 flex-1 group-hover/shelf:translate-x-2 transition-transform">
                                <h2 className="text-lg text-cyan-100/80 font-mono uppercase tracking-wider">{shelf.title} <span className="text-xs text-slate-500 ml-2">({totalItems})</span></h2>
                       ...
                       {/* Items Row */}
                       <div className="flex flex-wrap items-end gap-x-6 gap-y-16 pl-4 relative z-10 transition-all duration-500">
                       ...
                       {totalItems > PREVIEW_LIMIT && (
                          <div className="w-full flex justify-center mt-8">
                             <button 
                                ...
                                className="group flex items-center gap-2 px-6 py-2 rounded-full bg-slate-900/80 border border-cyan-900/50 hover:border-cyan-500 hover:bg-cyan-900/20 transition-all text-xs font-mono uppercase tracking-widest text-cyan-500"
                             >
                                {isExpanded ? (
                                   <>Thu gọn <ChevronUp size={14} className="group-hover:-translate-y-1 transition-transform" /></>
                                ) : (
                                   <>Xem toàn bộ (+{remainingCount} đĩa) <ChevronDown size={14} className="group-hover:translate-y-1 transition-transform" /></>
                                )}
                             </button>
                          </div>
                       )}
