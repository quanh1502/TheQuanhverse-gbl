...
import { 
  ...
  LayoutGrid, Library, Filter, ArrowUpDown, Heart, Sparkles, Share2,
  ChevronLeft
} from 'lucide-react';
...
// --- UPDATED COMPONENT: SPOTLIGHT HERO (CAROUSEL) ---
const SpotlightHero = ({ item, onClick, onNext, onPrev, total, currentIndex }: { item: AlbumItem, onClick: () => void, onNext: () => void, onPrev: () => void, total: number, currentIndex: number }) => {
    ...
    // Key change forces React to re-trigger animation when item changes
    return (
        <div className="relative w-full overflow-hidden rounded-3xl mb-12 group cursor-pointer shadow-2xl animate-appear-from-void min-h-[350px]">
            ...
            {total > 1 && (
                <>
                    <div onClick={(e) => {e.stopPropagation(); onPrev();}} className="absolute left-4 top-1/2 -translate-y-1/2 z-30 p-2 bg-black/30 hover:bg-black/60 backdrop-blur-md rounded-full text-white/50 hover:text-white transition-all cursor-pointer hover:scale-110">
                        <ChevronLeft size={24} />
                    </div>
                    <div onClick={(e) => {e.stopPropagation(); onNext();}} className="absolute right-4 top-1/2 -translate-y-1/2 z-30 p-2 bg-black/30 hover:bg-black/60 backdrop-blur-md rounded-full text-white/50 hover:text-white transition-all cursor-pointer hover:scale-110">
            ...
            <div key={`bg-${item.id}`} className="absolute inset-0 z-0 transition-opacity duration-700">
                <img src={item.coverUrl} alt="" className="w-full h-full object-cover blur-[80px] opacity-60 scale-150 animate-pulse-glow" />
            ...
            <div key={item.id} className="relative z-10 p-6 md:p-10 flex flex-col md:flex-row items-end md:items-center gap-6 md:gap-10 h-full animate-slide-in" onClick={onClick}>
                <div className="w-40 h-40 md:w-56 md:h-56 shrink-0 shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-lg overflow-hidden transform transition-transform duration-500 group-hover:scale-105 group-hover:rotate-2 border border-white/10">
                    <img src={item.coverUrl} alt={item.title} className="w-full h-full object-cover" />
                </div>
                <div className="flex-1 space-y-3 pb-2">
                    <div className="flex items-center gap-2 mb-2">
                        <span className="bg-cyan-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-1 shadow-[0_0_15px_rgba(6,182,212,0.6)]">
                           <Sparkles size={10} fill="currentColor"/> Editor's Choice
                        </span>
                        {total > 1 && <span className="text-[10px] text-slate-400 font-mono tracking-widest">{currentIndex + 1} / {total}</span>}
                    </div>
                    <h2 className="text-3xl md:text-5xl font-bold text-white leading-tight drop-shadow-lg group-hover:text-cyan-200 transition-colors line-clamp-2">{item.title}</h2>
                    <div className="flex items-center gap-3 text-lg md:text-xl text-slate-300 font-medium">
                        <span>{item.artist}</span>
                        <span className="w-1 h-1 rounded-full bg-slate-500"></span>
                        <span className="text-sm font-mono opacity-70">{item.year}</span>
                    </div>
                    {item.description && (
                        <p className="text-sm text-slate-400 italic max-w-xl line-clamp-2 opacity-80 border-l-2 border-cyan-500/50 pl-3 mt-2">"{item.description}"</p>
                    )}
                    <div className="pt-4 flex items-center gap-3 opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                         ...
                         <span className="text-sm font-bold text-white uppercase tracking-wider">Nghe Ngay</span>
                    </div>
                </div>
            </div>
            ...
            {total > 1 && (
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20">
                    {Array.from({ length: Math.min(total, 5) }).map((_, idx) => ( // Chỉ hiện max 5 dot
                        <div key={idx} className={`w-1.5 h-1.5 rounded-full transition-all ${idx === (currentIndex % 5) ? 'bg-cyan-400 w-4' : 'bg-white/20'}`}></div>
                    ))}
                </div>
            )}
        </div>
...
// (Giữ nguyên JewelCase3D, AddNewAlbum, DetailModal, EditModal như cũ)
        ...
        <div className="relative w-full max-w-3xl bg-slate-900/50 border border-white/10 rounded-3xl shadow-[0_0_80px_rgba(8,145,178,0.15)] overflow-hidden animate-zoom-in flex flex-col md:flex-row z-[105] backdrop-blur-2xl">
            ...
            <div className="flex-1 p-8 md:p-10 flex flex-col relative">
                ...
                <div className="mt-8 pt-6 border-t border-white/5">
                  {item.trackUrl ? ( <a href={item.trackUrl} target="_blank" rel="noopener noreferrer" className="group flex items-center justify-between w-full p-4 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white rounded-2xl font-bold transition-all shadow-lg hover:shadow-cyan-500/25 transform hover:-translate-y-1"><span className="pl-2">{item.description?.includes("Gợi ý") ? "Nghe thử trên Youtube" : "Play Music"}</span><div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center group-hover:bg-white group-hover:text-cyan-600 transition-colors"><Play size={18} fill="currentColor" /></div></a> ) : ( <div className="text-center text-slate-500 text-sm italic py-4">No link available</div> )}
                </div>
            </div>
        </div>
    </div>
...
const AudioRoom: React.FC<AudioRoomProps> = ({ initialMood }) => {
  ...
  // --- GET "SPOTLIGHT" ITEMS (ALL FAVORITES) ---
  const spotlightItems = useMemo(() => {
      const allFavorites: AlbumItem[] = [];
      shelves.forEach(shelf => {
          shelf.items.forEach(item => {
              if(item.isFavorite) allFavorites.push(item);
          });
      });
      // Sắp xếp bài mới lên đầu
  ...
  // --- CAROUSEL LOGIC ---
  useEffect(() => {
      // Tự động chuyển slide sau 8s nếu có nhiều hơn 1 bài
  ...
  const allTracks = useMemo(() => {
      ...
      shelves.forEach(shelf => {
          shelf.items.forEach(item => { tracks.push({ item, shelfId: shelf.id }); });
      });
    ...
    <div className="relative h-full w-full flex flex-col items-center bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#0f172a] to-black overflow-hidden text-slate-200">
      ...
      <div className="relative w-full h-full overflow-y-auto scrollbar-hide px-4 pb-32 z-10">
         <div className="max-w-7xl mx-auto min-h-[500px]">
             ...
             {/* SHELVES & LIBRARY VIEWS (Giữ nguyên) */}
                    ...
                    {shelves.map((shelf) => (
                        <div key={shelf.id} ref={(el) => { if (el) shelfRefs.current.set(shelf.id, el); }} className="relative group transition-all duration-500" onDragOver={(e) => { e.preventDefault(); }} onDrop={(e) => handleDrop(e, shelf.id)}>
                            ...
                            <div className="flex flex-wrap items-end gap-x-8 gap-y-12 pl-4">
                                {shelf.items.slice(0, PREVIEW_LIMIT).map((item, index) => ( <div key={item.id} draggable onDragStart={(e) => handleDragStart(e, item, shelf.id, index)} onDragEnd={handleDragEnd} onDrop={(e) => { e.stopPropagation(); handleDrop(e, shelf.id, index); }}> <JewelCase3D item={item} onClick={() => setViewingItem(item)} onEdit={() => setEditingItem({ item, shelfId: shelf.id })} /> </div> ))}
             ...
             {viewMode === 'library' && !focusedShelfId && (
                 <div className="py-8 animate-fade-in">
                     ...
                     <div className="flex flex-wrap items-end justify-center gap-x-10 gap-y-16"> {allTracks.length === 0 ? ( <div className="text-slate-500 italic py-20">Không tìm thấy bài hát nào.</div> ) : ( allTracks.map(({item, shelfId}) => ( <JewelCase3D key={item.id} item={item} onClick={() => setViewingItem(item)} onEdit={() => setEditingItem({ item, shelfId })} /> )) )} </div>
                 </div>
             )}
             {focusedShelf && (
                <div className="animate-zoom-in py-8">
                    ...
                    <div className="flex flex-wrap items-end justify-center gap-x-10 gap-y-16"> {focusedShelf.items.map((item, index) => ( <div key={item.id} draggable onDragStart={(e) => handleDragStart(e, item, focusedShelf.id, index)} onDragEnd={handleDragEnd} onDrop={(e) => { e.stopPropagation(); handleDrop(e, focusedShelf.id, index); }}> <JewelCase3D item={item} onClick={() => setViewingItem(item)} onEdit={() => setEditingItem({ item, shelfId: focusedShelf.id })} /> </div> ))} <AddNewAlbum onClick={() => handleAddNewItem(focusedShelf.id)} /> </div>
                </div>
             )}
         </div>
      </div>
