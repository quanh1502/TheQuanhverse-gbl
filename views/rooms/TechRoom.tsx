...
const EMOTIONS = [
  { 
    ...
    color: '#FFD700', 
    type: 'good',
    placeholder: "Điều gì đang thắp sáng hy vọng trong bạn?",
    effectDesc: "Hóa thành ngôi sao dẫn lối và thắp sáng bầu trời..."
  },
  { 
    id: 'sad', 
    label: 'Nỗi Buồn', 
    color: '#3498DB', 
    type: 'heavy',
    placeholder: "Nỗi buồn nào đang trĩu nặng trong lòng?",
    effectDesc: "Thấm sâu vào rễ cây, biến thành sự trầm mặc..."
  },
  { 
    id: 'anger', 
    label: 'Xả Giận', 
  ...
  { 
    ...
    color: '#2ECC71', 
    type: 'good',
    placeholder: "Chia sẻ niềm vui nhỏ bé hôm nay...",
    effectDesc: "Cây sẽ nhảy múa và rải hạt hạnh phúc..."
  },
  { 
    id: 'dream', 
    label: 'Giấc Mơ', 
    color: '#9B59B6', 
    type: 'good',
    placeholder: "Bạn đang mơ mộng về điều gì?",
    effectDesc: "Bay lơ lửng và dệt nên những giấc mơ..."
  },
  { 
    id: 'empty', 
    label: 'Trống Rỗng', 
    color: '#BDC3C7', 
    type: 'heavy',
    placeholder: "Nếu không biết nói gì, cứ để khoảng trống lên tiếng...",
    effectDesc: "Lặng lẽ tan biến vào hư không..."
  } 
];
    ...
    const render = () => {
      ...
      if (state.activeEffect === 'anger') {
          if (state.effectTimer < 750) {
              ctx.save();
              ctx.globalAlpha = Math.min(1, (750 - state.effectTimer) * 0.05);
          ...
          for (let i = state.fireworks.length - 1; i >= 0; i--) {
              ...
              f.x += f.vx; f.y += f.vy; 
              f.vy += 0.1; f.life -= 1.5; f.alpha -= 0.02;
      ...
      for (let i = state.projectiles.length - 1; i >= 0; i--) {
          ...
          if (p.progress >= 1) {
              ...
              if (p.type === 'anger') {
                  state.activeEffect = 'anger'; state.effectTimer = 800; 
      ...
      for (let i = state.blooms.length - 1; i >= 0; i--) {
        ...
        if (b.type === 'falling') {
            b.y += b.vy; 
            b.x += Math.sin(state.time + b.id) * 1; 
    ...
    <div className="relative w-full h-screen bg-[#020202] overflow-hidden font-sans text-white">
      ...
      <div className="absolute bottom-0 left-0 w-full pb-6 pt-10 md:pb-8 md:pt-20 px-4 flex flex-col items-center justify-end z-50 bg-gradient-to-t from-black via-black/90 to-transparent pointer-events-none">
        <div className="w-full max-w-[500px] flex flex-col items-center gap-3 md:gap-5 pointer-events-auto">
            ...
            <div className="flex justify-center gap-3 md:gap-4 p-2 flex-wrap">
                {EMOTIONS.map((mood) => (
                    <button
                        ...
                        className={`w-10 h-10 md:w-10 md:h-10 rounded-full border-2 transition-all duration-200 ${
            ...
            {/* INPUT & SEND */}
            <div className="w-full relative flex items-center gap-2 bg-white/5 backdrop-blur-xl border border-white/10 rounded-full px-3 py-3 md:px-4 md:py-3 shadow-2xl focus-within:bg-white/10 focus-within:border-white/30 transition-colors" style={{borderColor: currentEmotion.color + '40'}}>
                <input 
                    type="text"
                    value={inputValue}
                    onChange={(e) => setInputValue(e.target.value)}
                    onKeyDown={(e) => e.key === 'Enter' && sendToVoid()}
                    // Placeholder thay đổi theo mood
                    placeholder={currentEmotion.placeholder}
                    className="flex-1 bg-transparent border-none text-white/90 font-serif text-base md:text-lg px-2 md:px-4 focus:outline-none placeholder:text-white/30 transition-all"
                />
                
                {/* Send Button đổi màu theo mood */}
                <button 
                  onClick={sendToVoid}
                  disabled={isWindBlowing}
                  className="p-2 md:p-3 rounded-full text-white transition-all hover:scale-105 active:scale-95 disabled:opacity-50 shadow-lg"
                  style={{ backgroundColor: currentEmotion.color }}
                >
                  <Send size={20} fill="rgba(0,0,0,0.2)" stroke="white" />
                </button>
            </div>
            
            {/* MÔ TẢ HIỆU ỨNG (HINT) */}
            <div 
                className="text-[10px] md:text-xs font-light tracking-widest uppercase mt-1 transition-all duration-500 h-4 text-center"
                style={{ color: currentEmotion.color }}
            >
                {currentEmotion.effectDesc}
            </div>
        </div>
      </div>
    </div>
